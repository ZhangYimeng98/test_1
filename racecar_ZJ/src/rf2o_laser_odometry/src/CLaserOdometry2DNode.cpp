...
bool ZJ_RF2O_output_error = false;
...
bool CLaserOdometry2DNode::setLaserPoseFromTf()
{
  ...
  try
  {
    last_scan.header.frame_id="/laser_link";
    tf_listener.waitForTransform(base_frame_id,last_scan.header.frame_id, ros::Time::now(), ros::Duration(20.0));
    tf_listener.lookupTransform(base_frame_id, last_scan.header.frame_id, ros::Time(0), transform);
    retrieved = true;
  }
  catch (tf::TransformException &ex)
  {
    if(ZJ_RF2O_output_error)
    {
      ROS_ERROR("%s",ex.what());
    }
...
void CLaserOdometry2DNode::publish()
{
  ...
  if (publish_tf)
  {
    ROS_INFO_STREAM("[rf2o] Publishing TF: [base_link] to [odom]");
